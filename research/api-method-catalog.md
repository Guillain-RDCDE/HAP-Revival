...
| `setPlayContent` | **1.1** | ✅ | `[{uri: "audio:track?id=NNN", output: ""}]` | **CONFIRMED at v1.1** (was Unsupported Version at v1.0). Returned `{result: []}` with empty params — server accepted. This is the playback initiation method. Untested with real URI yet. |
| `scanPlayingContent` | **1.0** | 🟡 | unknown | Exists; `[3, "illegal Argument"]`. Likely scrub/seek within current track. |
| `getContentInfo` | **1.1** | 🟡 | `[{uri: "audio:album?id=N"}]` | Exists; `[1, "Any"]` with empty params. Returns metadata for any URI. |
| `getContentList` | **1.3** | 🟡 | `[{uri: "audio:album", stIdx: 0, cnt: 10}]` | **Found at v1.3!** (was Unsupported at 1.0/1.2). `[3, "illegal Argument"]` with empty params. Needs correct shape — try concrete URIs. |
