...
- [**Fix it**](tools/hap_fixit.py) — the audit's findings with the **real folder each one is in**,
  ready to open in Explorer or a tag editor. If the album is also on your own disk it opens **your**
  copy — found by the sync mapping, or by file name if your folders are filed differently — so the
  edit is a local write and the next sync carries it. With no local library it simply opens the
  player's copy. Available as a tab in HAP Sync, a panel in the web UI, or a standalone HTML report.
