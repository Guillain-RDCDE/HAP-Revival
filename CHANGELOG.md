...
- **An audiophile audit without a screwdriver.** `library_audit.py --from-player <ip>` gives the
  hi-res / DSD / duplicate / missing-artwork report that previously needed the disk out of the
  machine.
- **[`hap_fixit.py`](tools/hap_fixit.py) — every finding with the folder it is in**, ready to open
  in Explorer or a tag editor, offered as a tab in HAP Sync, a panel in the web UI and an HTML
  report. Where the album is also in a folder you sync, it opens *your* copy, so the edit is local
  and the next sync carries it.
- **The front panel over HTTP** — fetch the 480×272 display as a PNG and inject its keys:
  [`hap_screen.py`](tools/hap_screen.py). A third API on port 60200, in no public documentation.
- **Internet radio.** Browse the player's TuneIn directory and play any station, which Sony's own
  app can no longer do.
- **Push notifications over UDP**, so the web UI stops polling and reacts the instant anything
  changes.
- [`docs/16-gotchas.md`](docs/16-gotchas.md) — nine cases where the generally correct move is the
  locally wrong one on this device.
- [`hap_intercept.py`](tools/hap_intercept.py), a logging resolver for learning what the player
  looks up; `check_links.py` in CI; the mock device now serves the REST catalogue and the front
  panel, so every tool but the live smoke test runs with no hardware.

### Changed

- **The README, rewritten in two halves** — a numbered path for owners, then the technical case for
  anyone weighing whether to trust the project.
- [`.github/CONTRIBUTING.md`](.github/CONTRIBUTING.md) now says which machine can answer which
  question: an HAP-S1, an older firmware and a second region's TuneIn tree each answer things a
  HAP-Z1ES cannot.

### Fixed

- **The library API was never dead.** It answers fine; every tool here gave up after six seconds
  while cold requests take 5–57 s. Months of documentation said otherwise
  ([measurements](research/notes/2026-08-29-contentdb-was-never-dead.md)).
- **Timeouts calibrated on one library are still guesses.** The per-page deadline now doubles on
  retry instead of sitting at a constant measured against one 78 369-track collection.
- **A share crawl silently lost 90% of the second share** — one SMB connection reused across two
  long listings desyncs SMB1, and per-folder error tolerance swallowed it. 5 931 files instead of
  66 733, with no error.
- **Response bodies are not uniformly UTF-8**: one artist name in 17 317 arrived as raw Latin-1,
  and `json.loads` lost the whole 343 KB page over one character.
- A saturated catalogue entry was reported as a 1048 kHz hi-res track rather than as unreadable.
- Clicking a track in the library did nothing (`/api/play_track` vs `/api/play-track`), and two GUI
...
Six conclusions published here were overturned — five by a contributor who owns the hardware, one
by our own testing. Each is written up next to the claim it replaced:

1. TuneIn registration was never the gate.
2. The "local cache" explanation for radio playback was wrong.
3. Internet radio worked the whole time; we were sending one header too many.
4. The legacy page's console does not wait for a click — it unlocks on a hard-coded date.
5. Our `contentdb` was not broken while another player's was healthy.
6. `contentdb` was not dead anywhere. It is slow, and we were impatient.
